...
#        [--false-complete no] [--no-gate]
set -u
export LC_ALL=C.UTF-8

here=$(cd "$(dirname "$0")" && pwd)
cmd="" project="." auto=0 no_gate=0 max_iterations=12
task="" files="" cp="" cf="" cu="" progress=yes exit_signal=no errsig=none fcomplete=no
...
case "$cmd" in
  ...
  close-iteration)
    ...
    printf 'iteration=%s\nbreaker=%s\nfalse_completes=%s\nreplans=%s\nno_progress_streak=%s\nlast_progress_iteration=%s\nmax_iterations=%s\nno_progress_limit=%s\nmax_replans=2\nper_check_fail_cap=3\npanel_max=4\n' \
      "$new" "$breaker" "$fc" "$(state_get replans)" "$streak" "$lpi" "$mi" "$(state_get no_progress_limit)" > "$sd/state.rec"
